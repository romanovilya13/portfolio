...
ORDER BY 
  c.CategoryName, 
  s.SaleMonth;
/*В этом запросе мы используем следующие элементы:
...
В результате запроса мы получим отчет, который показывает продажи по категориям продуктов в разбивке по месяцам за последние 2 года, а также running total, продажи за предыдущий месяц и темпы роста продаж для каждой категории.*/
